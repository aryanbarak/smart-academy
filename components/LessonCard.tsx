import React from 'react';
import { LESSONS } from '../constants';

type LessonType = typeof LESSONS[number];

interface LessonCardProps {
  lesson: LessonType;
  index: number;
  isActive: boolean;
  isCompleted: boolean;
  isBookmarked: boolean;
  onToggle: () => void;
  onToggleComplete: () => void;
  onToggleBookmark: () => void;
  children?: React.ReactNode;
}

const LessonCard: React.FC<LessonCardProps> = ({
  lesson,
  index,
  isActive,
  isCompleted,
  isBookmarked,
  onToggle,
  onToggleComplete,
  onToggleBookmark,
  children,
}) => {
  const isGA2 = lesson.type === 'GA2';
  const isWISO = lesson.type === 'WISO';
  const isPRUEF = lesson.type === 'PRUEF';

  const accent =
    isGA2 ? 'blue' : isWISO ? 'emerald' : 'purple';

  const borderActive = `border-${accent}-500 ring-1 ring-${accent}-500/70 shadow-md`;
  const badgeAccentLight =
    accent === 'blue'
      ? 'bg-blue-50 text-blue-700 border-blue-100'
      : accent === 'emerald'
      ? 'bg-emerald-50 text-emerald-700 border-emerald-100'
      : 'bg-purple-50 text-purple-700 border-purple-100';

  const badgeAccentDark =
    accent === 'blue'
      ? 'dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-900/40'
      : accent === 'emerald'
      ? 'dark:bg-emerald-900/30 dark:text-emerald-300 dark:border-emerald-900/40'
      : 'dark:bg-purple-900/30 dark:text-purple-300 dark:border-purple-900/40';

  const accentIconBg =
    accent === 'blue'
      ? 'bg-blue-600'
      : accent === 'emerald'
      ? 'bg-emerald-600'
      : 'bg-purple-600';

  const titleActiveColor =
    accent === 'blue'
      ? 'text-blue-600 dark:text-blue-400'
      : accent === 'emerald'
      ? 'text-emerald-600 dark:text-emerald-400'
      : 'text-purple-600 dark:text-purple-400';

  return (
    <article
      className={`relative rounded-xl border bg-white/80 dark:bg-slate-900/80 transition-all duration-200 overflow-hidden group ${
        isActive
          ? borderActive
          : 'border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 hover:shadow-md'
      }`}
      style={{ animationDelay: `${index * 40}ms` }}
    >
      {/* Accent bar */}
      <div
        className={`absolute inset-y-0 left-0 w-1.5 rounded-tr-xl rounded-br-xl ${
          accent === 'blue'
            ? 'bg-gradient-to-b from-blue-500 via-sky-500 to-blue-600'
            : accent === 'emerald'
            ? 'bg-gradient-to-b from-emerald-500 via-teal-400 to-emerald-600'
            : 'bg-gradient-to-b from-purple-500 via-fuchsia-500 to-purple-600'
        }`}
      />

      <div className="flex items-stretch">
        {/* Main clickable area */}
        <button
          type="button"
          onClick={onToggle}
          className="flex-1 text-left px-5 py-4 md:py-5 flex items-center justify-between gap-4 focus:outline-none"
        >
          <div className="flex-1 pr-2">
            {/* Meta row */}
            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
              <span
                className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide border ${badgeAccentLight} ${badgeAccentDark}`}
              >
                {lesson.type === 'PRUEF' ? 'Pruefung' : lesson.type}
              </span>

              <span className="text-[11px] text-slate-400 font-mono">
                #{lesson.order}
              </span>

              {lesson.level && (
                <span className="text-[11px] font-medium text-slate-500 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">
                  {lesson.level}
                </span>
              )}

              {isCompleted && (
                <span className="text-[11px] font-medium text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 px-2 py-0.5 rounded-full inline-flex items-center gap-1">
                  <svg
                    className="w-3 h-3"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
                      fillRule="evenodd"
                      d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                      clipRule="evenodd"
                    />
                  </svg>
                  Abgeschlossen
                </span>
              )}

              {isBookmarked && (
                <span className="text-[11px] text-amber-600 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/30 px-2 py-0.5 rounded-full">
                  Markiert
                </span>
              )}
            </div>

            {/* Title & subtitle */}
            <h3
              className={`text-sm md:text-base font-semibold transition-colors ${
                isActive ? titleActiveColor : 'text-slate-900 dark:text-slate-50'
              }`}
            >
              {lesson.title}
            </h3>
            {lesson.subtitle && (
              <p className="mt-1 text-xs md:text-[13px] text-slate-500 dark:text-slate-300 leading-relaxed">
                {lesson.subtitle}
              </p>
            )}
          </div>

          {/* Arrow */}
          <div
            className={`flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 ${
              isActive
                ? `${accentIconBg} text-white shadow-sm rotate-180`
                : 'bg-slate-100 dark:bg-slate-800 text-slate-400 group-hover:text-slate-900 dark:group-hover:text-slate-50'
            }`}
          >
            <svg
              className="w-4 h-4"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M19 9l-7 7-7-7"
              />
            </svg>
          </div>
        </button>

        {/* Right action buttons */}
        <div className="flex items-center gap-1 pr-3 py-3">
          {/* Complete */}
          <button
            type="button"
            onClick={e => {
              e.stopPropagation();
              onToggleComplete();
            }}
            className={`p-2 rounded-full transition-colors ${
              isCompleted
                ? 'text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-900/30'
                : 'text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'
            }`}
            title={
              isCompleted
                ? 'Als nicht abgeschlossen markieren'
                : 'Als abgeschlossen markieren'
            }
          >
            <svg
              className="w-4 h-4"
              fill={isCompleted ? 'currentColor' : 'none'}
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"
              />
            </svg>
          </button>

          {/* Bookmark */}
          <button
            type="button"
            onClick={e => {
              e.stopPropagation();
              onToggleBookmark();
            }}
            className={`p-2 rounded-full transition-colors ${
              isBookmarked
                ? 'text-amber-500 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-900/30'
                : 'text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'
            }`}
            title={
              isBookmarked
                ? 'Lesezeichen entfernen'
                : 'Lesezeichen hinzufuegen'
            }
          >
            <svg
              className="w-4 h-4"
              fill={isBookmarked ? 'currentColor' : 'none'}
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"
              />
            </svg>
          </button>
        </div>
      </div>

      {/* Expanded content */}
      {isActive && children}
    </article>
  );
};

export default LessonCard;
